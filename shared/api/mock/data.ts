...
export const MOCK_ADVISOR_SUGGESTIONS = {
  suggestions: [
    {
      alarm_id: '46',
      alarm_name: '요금온라인 전체 서비스 시스템오류 10분간 30건 이상 발생',
      service_id: 'BG008802',
      service_name: 'KOS-요금온라인',
      alarm_level: 'Minor',
      detect_type: '호출건수',
      detect_term: '10분',
      current_threshold: 30,
      suggested_threshold: 50,
      auto_clear_rate: 0.87,
      trigger_count_30d: 23,
      avg_detect_value: 41.2,
      rationale: '최근 30일간 23회 발생했으나 20회(87%) 자동 해소됐습니다. 평균 탐지값(41)이 임계값(30)에 근접하여 실제 장애보다 정상 변동 범위 내 노이즈로 판단됩니다.',
      status: 'pending',
      approved_at: null,
      rejected_at: null,
    },
    {
      ...
      suggested_threshold: 4500,
      auto_clear_rate: 0.80,
      trigger_count_30d: 15,
      avg_detect_value: 3750,
      rationale: '최근 30일간 15회 발생했으나 12회(80%) 자동 해소됐습니다. 평균 탐지값(3750ms)이 임계값(3000ms)에 근접하여 실제 장애보다 정상 변동 범위 내 노이즈로 판단됩니다.',
      status: 'pending',
      approved_at: null,
      rejected_at: null,
    },
    {
      alarm_id: '191',
      alarm_name: 'NBSS_SMS B2C PO 1시간내 오류 10% 이상',
      service_id: 'BG009102',
      service_name: 'KOS-B2C CRM',
      alarm_level: 'Major',
      detect_type: '오류율',
      detect_term: '1시간',
      current_threshold: 10,
      suggested_threshold: 15,
      auto_clear_rate: 0.73,
      trigger_count_30d: 11,
      avg_detect_value: 12.4,
      rationale: '최근 30일간 11회 발생했으나 8회(73%) 자동 해소됐습니다. 평균 탐지값(12.4%)이 임계값(10%)에 근접하여 실제 장애보다 정상 변동 범위 내 노이즈로 판단됩니다.',
      status: 'approved',
      approved_at: new Date(Date.now() - 2 * 60 * 60_000).toISOString(),
      rejected_at: null,
    },
  ],
  totalCount: 3,
  pendingCount: 2,
  lastAnalyzedAt: new Date(Date.now() - 30 * 60_000).toISOString(),
};
